...
nant            = 15 # Total number of antenna. Will make an automatic checkup for this in the future.
#
#####################
# Calibration files #
#####################
...
#
## For accor
accor_smooth_ant=[False] #set to False if all antennas schould be smoothed or to a list of antennas which should be smoothed
...
########################
# Here as example to correct mounting for PV, assuming it is antennas 14
tabed_ine		= ['AN']
tabed_optype	= ['REPL']
tabed_keyvalue	= [[5,0]]
...
###########################
apcal_dofit     = nant*[1] #asuming 15 antennas
apcal_tyv       = 1
apcal_tau0      = nant*[0.1]
apcal_trecvr    = 2*nant*[100]
...
# Parameters from which cl and sn table  should be deleted if using clear_manual_phasecal and not wanting to use the automatic algorithm for getting cl-high and sn_high.
# if the above assumption of finding the first SN and CL tables produced by manual phasecal does not work as suspected, the last CL and SN tables that should not be deleted can be specified here, typically this should not be needed
...
bandpass_correction = False # apply a bandpass (autocorrelation one)
