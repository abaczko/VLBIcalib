...
def get_antenna_number(uvdata,antenna):
	...
	for i in range(len(antennas)):
		if antennas[i]==antenna:
			...
			return i+1
#
def get_several_listings(uvdata):
	'''
	Prints out MATX with different parameter setups.
	'''
	AT.listr(uvdata,optype='MATX',sources=OP.calibrators,bif=1,eif=1,dparm=[3,0])
	AT.listr(uvdata,optype='MATX',sources=OP.calibrators,bif=1,eif=1,dparm=[4,0])
	AT.listr(uvdata,optype='MATX',sources=OP.calibrators,bif=1,eif=1,dparm=[5,0])
	AT.listr(uvdata,optype='MATX',sources=OP.calibrators,bif=1,eif=1,dparm=[14,0])
...
def	do_global_fring(uvdata,timer,aparm,dparm,refant,cals=[[]],sources=[[]],antennas=[0],interpol=['2PT'],search=[0],solint=[2],gainu=[0],suba=[0],dofit=[0],solmin=[0],solsub=[0],dosnsmo=False,bpv=0,doband=-1,get2n=False):
	...
	print len(timer)
	for i in np.arange(0,len(timer)):
